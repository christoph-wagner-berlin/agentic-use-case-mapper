...
- **Business Needs & Patterns** page — steps back from tool-mapped use cases: browsable **business needs** (raw problems, not yet mapped to any tool) and **tool-agnostic use case patterns** (proven generic solution shapes independent of any vendor), with links showing which needs match which patterns and which patterns are implemented by which specific tool-mapped use cases. Starts empty — schema and seeding are wired up, ready for real entries to be added to `data/seed/business_needs_seed.csv` / `tool_agnostic_use_cases_seed.csv` and their link CSVs.
